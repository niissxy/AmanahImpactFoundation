...
export default function ZakatCalculator({ currentLang, onPayZakat, langPack }: ZakatCalculatorProps) {
  ...
  return (
    <div id="zakat-calculator" className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-5 md:p-6 max-w-2xl mx-auto animate-in fade-in zoom-in-95 duration-200">
      {/* Icon and title header */}
      <div className="flex items-center gap-3.5 border-b border-gray-100 dark:border-gray-700 pb-4 mb-5">
        <div className="p-2.5 bg-emerald-105 dark:bg-emerald-900/60 text-emerald-600 dark:text-emerald-400 rounded-xl shrink-0">
      ...
      <div className="bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/40 rounded-xl p-3.5 mb-5 flex flex-wrap sm:flex-nowrap items-center gap-3.5">
      ...
      {/* Tab Selectors */}
      <div className="grid grid-cols-2 gap-1.5 bg-gray-50 dark:bg-gray-900 p-1 rounded-xl mb-5">
        <button
          id="btn-zakat-salary"
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="space-y-3.5">
              ...
              <div>
                ...
                <div className="relative rounded-md shadow-sm">
                  ...
                  <input
                    ...
                    className="block w-full pl-7 pr-2.5 py-1.5 border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-lg text-xs focus:outline-none focus:ring-1.5 focus:ring-emerald-500"
                    placeholder="7500000"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative rounded-md shadow-sm">
                  <input
                    ...
                    className="block w-full pr-8 pl-2.5 py-1.5 border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-lg text-xs focus:outline-none focus:ring-1.5 focus:ring-emerald-500"
                    placeholder="0"
                  />
          ...
          <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-3 border border-gray-200 dark:border-gray-800 text-[11px] text-gray-500 dark:text-gray-400 space-y-1">
            ...
            {activeTab === 'income' ? (
              <p>
                {t('Nisab Zakat Profesi bulanan setara harga **522 kg makanan pokok/beras** atau setara **85 Gram Emas per tahun dibagi 12 bulan** (sekitar')} {currentMonthNisabFormated} {t('per bulan). Jika pendapatan bersih bersisa di atas nisab wajib menyisihkan 2,5% zakat.')}
              </p>
            ) : (
              <p>
                {t('Nisab Zakat Maal/Tabungan tahunan setara harga **85 Gram Emas** (sekitar')} {currentYearNisabFormated}{t('). Jika total aset mengendap setahun melampaui nisab wajib mengeluarkan zakat tabungan 2,5%.')}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-slate-900 rounded-xl p-5 border border-emerald-100/50 dark:border-emerald-900/30 flex flex-col justify-between">
          <div className="space-y-3">
            <h4 className="text-[11px] font-black uppercase tracking-wider text-emerald-800 dark:text-emerald-450 border-b border-emerald-100 dark:border-emerald-900/50 pb-2">
            ...
            <div className="flex items-center justify-between bg-white dark:bg-gray-800 rounded-lg p-2.5 shadow-sm border border-emerald-100/30">
              <span className="text-xs text-gray-500 dark:text-gray-300 font-semibold">{t('Apakah Nisab Terpenuhi?')}</span>
              {isNisabMet ? (
                <span className="bg-emerald-100 dark:bg-emerald-900/80 text-emerald-800 dark:text-emerald-300 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                  <CheckCircle className="w-3 h-3" /> {t('YA (Wajib Zakat)')}
                </span>
              ) : (
                <span className="bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                  <Info className="w-3 h-3" /> {t('TIDAK (Belum Wajib)')}
                </span>
              )}
            </div>

            {/* Total due cash */}
            <div className="bg-emerald-600 dark:bg-emerald-800 rounded-xl p-4 text-white flex flex-col justify-center shadow-md">
              <span className="text-[10px] text-emerald-100 uppercase tracking-widest font-black">{t('Total Zakat yang Harus Ditunaikan')}</span>
              <span className="text-2xl font-black tracking-tight mt-0.5">{formatCurrency(totalZakat)}</span>
            </div>

            {/* Intention Recitation Panel */}
            {isNisabMet && totalZakat > 0 && (
              <div id="zakat-niat-panel" className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800 rounded-lg p-3 space-y-1.5 mt-2 text-[11px]">
          ...
          <div className="mt-4 pt-3 border-t border-emerald-100/50 dark:border-emerald-900/50">
            {totalZakat > 0 ? (
              <button
                id="btn-confirm-pay-zakat"
                onClick={() => onPayZakat(totalZakat, activeTab === 'income' ? 'Zakat Penghasilan' : 'Zakat Maal')}
                className="w-full bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 dark:bg-emerald-500 dark:hover:bg-emerald-600 text-white font-bold py-2.5 px-4 rounded-xl shadow-md transition-colors text-xs flex items-center justify-center gap-1.5 cursor-pointer"
              >
              ...
              <button
                ...
                className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-white font-bold py-2.5 px-4 rounded-xl border border-gray-200 dark:border-gray-700 transition-colors text-xs flex items-center justify-center gap-1"
              >
